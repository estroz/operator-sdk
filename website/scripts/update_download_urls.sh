...
if ! grep -q "OPERATOR_SDK_DL_URL=https://github\.com/operator-framework/operator-sdk/releases/${VERSION}" "$DOC_PATH"; then
  echo "$0 failed to update ${DOC_PATH}"
  exit 1
fi

CONFIG_PATH="${DIR}/../../netlify.toml"
TARGET="(https://sdk\.operatorframework\.io/dl/)v[^/]+(.+)"

sed -i -E 's@'"${TARGET}"'@\1'"${VERSION}"'\2@g' "$CONFIG_PATH"

# Ensure the file was updated.
if ! grep -q "https://sdk\.operatorframework\.io/dl/${VERSION}" "$CONFIG_PATH"; then
  echo "$0 failed to update ${CONFIG_PATH}"
  exit 1
fi
