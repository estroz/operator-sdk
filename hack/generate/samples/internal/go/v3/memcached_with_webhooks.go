...
func (mh *MemcachedGoWithWebhooks) uncommentDefaultKustomizationFile() {
	log.Infof("uncomment config/default/kustomization.yaml to enable webhook and ca injection")
	kustomizationPath := filepath.Join(mh.ctx.Dir, "config", "default", "kustomization.yaml")
	var err error

	err = testutils.UncommentCode(kustomizationPath, "#- ../webhook", "#")
	pkg.CheckError("uncomment webhook", err)

	err = testutils.UncommentCode(kustomizationPath, "#- ../certmanager", "#")
	...
	err = testutils.UncommentCode(kustomizationPath, "#- manager_webhook_patch.yaml", "#")
	pkg.CheckError("uncomment manager_webhook_patch.yaml", err)

	err = testutils.UncommentCode(kustomizationPath, "#- webhookcainjection_patch.yaml", "#")
	pkg.CheckError("uncomment webhookcainjection_patch.yaml", err)

	err = testutils.UncommentCode(kustomizationPath,
